...
class Babelstream(CMakePackage, CudaPackage, ROCmPackage):
    ...
    def cmake_args(self):
        ...
        if ("+stddata" in self.spec): #  spec_string.startswith(tuple(std_list)):
            args.append("-DCMAKE_CXX_COMPILER=/projects/bristol/modules/gcc/13.1.0/bin/g++")
            print(spack_cxx)
        ...
        if ("+omp" in self.spec) and ("~kokkos" in self.spec) and ("~raja" in self.spec):
            print("TEST")
            # args.append("-DCMAKE_CXX_COMPILER=/lustre/home/br-kolgu/spack/opt/spack/cray-rhel8-cascadelake/gcc-12.3.0/nvhpc-23.3-mkhqfqdneblshcylgbgsnuen5tyyxggy/Linux_x86_64/2023/compilers/bin/nvc++")# + self.compiler.cxx)
            # print(self.compiler.cxx)
            args.append("-DCMAKE_CXX_COMPILER=" + spack_cxx)
            # if "cuda_arch" in self.spec.variants:
                # cuda_arch_list = self.spec.variants["cuda_arch"].value
                # the architecture value is only number so append sm_ to the name
                # cuda_arch = "cc_" + cuda_arch_list[0]
            args.append("-DOFFLOAD=ON")
            args.append("-DOFFLOAD_FLAGS=-mp=gpu;-gpu=cc70")
